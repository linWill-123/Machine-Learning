...
import numpy as np
from plot_util import make_dict, parse_file, get_matrices, write_predictions, write_metrics
import argparse
import logging
...
def main(args):
    ...
    sentences_tr, tags_tr = parse_file(args.train_input)
    ...
    #######################################################
    avg_log_likelihood_tr = []

    for sequence in sentences_tr:
        # each sequence needs to run forward backward
        dpAlpha = [None for _ in range(len(sequence))]
        dpBeta = [None for _ in range(len(sequence))]
        '''Foreward'''
        ...
        # compute predicted tags and log likelihoods
        log_prob = logSumExpTrick(dpAlpha[len(dpAlpha) - 1])
        avg_log_likelihood_tr.append(log_prob)
    avg_log_likelihood_tr = sum(avg_log_likelihood_tr)/len(avg_log_likelihood_tr)
    ...
    print("validation log: ",avg_log_likelihood)
    print("training log: ",avg_log_likelihood_tr)
    return
