...
def logSumExpTrick(v):
    m = max(v)
    return m + np.log(np.sum(np.exp(v_i-m)))
'''

def minBayes(t, alpha, beta, total_weights, tag_dict):
    ...
    for sequence in sentences:
        ...
        for t in range(len(sequence)): #for t in T
            if t != 0:
                ...
                A = emission[:,emmisionIndex]
                B = transition
                W = B.T * A[:,np.newaxis] # flips B to get rows = current state, cols = prev state
                alpha_t = np.matmul(W, np.array(dpAlpha[t - 1]))
            else: # initialization
                emmisionIndex = word_dict[sequence[t]]
                A = emission[:,emmisionIndex]
                pi = init
        ...
        for t in range(len(sequence) - 1, -1, -1):
            if t != len(sequence) - 1:
                ...
                W = np.matmul(B, A)
                print("W ", W)
                print("previous beta", dpBeta[t+1])
                beta_t = W * dpBeta[t + 1]
                print("BETA: ",beta_t)
            else:
                W = np.ones(len(tag_dict))
                beta_t = W
            dpBeta[t] = beta_t
        count += 1
        print("Alpha: ",dpAlpha)
        ...
        total_weights = np.sum(dpAlpha[len(dpAlpha) - 1])
        # log
        log_prob = np.log(total_weights)
