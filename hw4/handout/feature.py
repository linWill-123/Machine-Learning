...
def bag_of_word(dataset, d, outfile):
    ...
    for i in range(len(dataset)):
        ...
        res.append(np.concatenate(([label], vectorizedString)))
    res = np.array(res)
    np.savetxt(outfile, res, delimiter = '\t', fmt = "%d")
def word_embedding(dataset, d, featuresize, outfile):
    ...
    for i in range(len(dataset)):
        ...
        res.append(np.concatenate(([label], vectorArray)))
    res = np.array(res)
    np.savetxt(outfile, res, delimiter = '\t', fmt = "%.6f")
