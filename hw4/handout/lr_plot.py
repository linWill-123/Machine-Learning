...
if __name__ == '__main__':
    ...
    validX = load_formatted_data(formatted_validation_in)
    y = np.copy(validX[:,0]) #labels col
    theta = np.zeros(len(validX[0]))#size of theta = number of features + intercept
    model,allModels = train(theta, validX, y, num_epoch, learning_rate)
    avgValRes = []

    for a_model in allModels:
        avgValRes.append(compute_avg(a_model, X))
    ...
    plt.plot(xLabels, avgTrainRes, color = 'green', label = "Avg Training Log")
    plt.plot(xLabels, avgValRes, color = 'red', label = "Avg Validation Log")
    plt.legend()
    plt.show()
